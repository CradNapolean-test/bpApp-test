...
// Folded in from the old FormsHubShell (was its own AppShell page). The list+editor duo is
// now a self-contained responsive pane: side-by-side on desktop, single-pane-with-back on
// mobile via local `mobileView` state -- no routing, no AppShell sidebar slot needed (that
// slot is now Library's dropdown switcher).
export function FormsPane({ initialTemplates }: { initialTemplates: FormTemplateRow[] }) {
  ...
  return (
    <div className="flex flex-col gap-4 md:flex-row md:gap-6">
      <nav className={`space-y-1 md:w-52 md:shrink-0 ${mobileView === 'editor' ? 'hidden md:block' : 'block'}`}>
        <button
          onClick={resetForm}
      ...
      <div className={`min-w-0 flex-1 ${mobileView === 'list' ? 'hidden md:block' : 'block'}`}>
        <button
          ...
          className="mb-3 text-sm font-medium text-zinc-500 md:hidden"
        >
          ← Back to templates
        </button>
